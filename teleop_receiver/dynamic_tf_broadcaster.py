...
def convert_to_radians_per_second(arduino_value):
    # Convert encoder ticks to radians per second
    encoder_ticks_per_rev = 1440  # Encoder ticks per revolution
    return (arduino_value / encoder_ticks_per_rev) * (2 * np.pi)
...
def compute_velocity(wheel_front_left, wheel_front_right, wheel_back_left, wheel_back_right):
    ...
    vx = (((convert_to_radians_per_second(wheel_front_left) + convert_to_radians_per_second(wheel_front_right) + convert_to_radians_per_second(wheel_back_left) + convert_to_radians_per_second(wheel_back_right))/4)/1440) * (r * 2 * np.pi)
    vy = (((-convert_to_radians_per_second(wheel_front_left) + convert_to_radians_per_second(wheel_front_right) + convert_to_radians_per_second(wheel_back_left) - convert_to_radians_per_second(wheel_back_right))/4)/1440) * (r * 2 * np.pi)

    # Compute the angular velocity, accounting for both length (L) and width (W)
    vtheta = math.pi * (r / (4 * (L + W))) * (-convert_to_radians_per_second(wheel_front_left) + convert_to_radians_per_second(wheel_front_right) - convert_to_radians_per_second(wheel_back_left) + convert_to_radians_per_second(wheel_back_right)) / 1440
